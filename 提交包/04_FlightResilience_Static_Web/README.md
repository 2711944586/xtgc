...
新版页面已取消单独现场入口模块，答辩现场直接使用正式访问地址或本地预览地址。前端字体统一为 Noto Serif SC / Noto Sans SC / JetBrains Mono，控制台界面使用更圆润的面板、图表、按钮和提示层。“自动演示”约 55 秒，会沿“问题-方法-证据-结论-当前操作”讲解轨道推进，并按真实交互触发选择、点击、网络节点和滑杆拖动。
...
3. 预测：调节小时、距离和拥堵水平，说明风险输入的变化和概率条的即时反馈。
4. 网络：点击机场节点，说明关键性不等同于单纯航班量。
5. 仿真：切换天气冲击、冲击机场和四类策略，比较恢复曲线、指标柱状图与恢复热力图。
6. 决策：调节 λ 权重并点击风险-TOPSIS 点，展示 dynamic_combo 和 baseline 的排名反转边界。
